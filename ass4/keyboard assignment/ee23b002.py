...
import dvorak_layout
import colemak_layout
...
def plot(
    ...
    """
    Function plots graph with animation
    """
    heatmap = np.zeros(grid_size)  # heatmap matrix for each pixel
    x_min, x_max = 0, 14.5  # coordinate limits
    y_min, y_max = 1, 5

    # setting up grid for each pixel for heatmap
    ...
    # Generating the colormap (blue to red)
    ...
    def update(frame):
        # function to update each frame for animation
        nonlocal heatmap  # non local variable (outside scope of function)
        nonlocal artist
        artist.remove()
        i = frame
    ...
    ani = FuncAnimation(
        fig, update, frames=len(x), interval=0, blit=False, repeat=False
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "-a":  # animaiting if -a flag given
            plot(x, y, grid_size, ax, fig)
        if sys.argv[1] == "-A":  # analysing for best layout
            distance_for_layouts = {
                "qwerty": caculate_key_travel(
                    inputStr, qwerty_layout.keys, qwerty_layout.characters
                ),
                "dvorak": caculate_key_travel(
                    inputStr, dvorak_layout.keys, colemak_layout.characters
                ),
                "colemak": caculate_key_travel(
                    inputStr, colemak_layout.keys, colemak_layout.characters
                ),
            }
            min_value = distance_for_layouts["qwerty"]
            most_efficient_layout = "qwerty"

            # checking the most efficient layout
            for layout in distance_for_layouts:
                if distance_for_layouts[layout] <= min_value:
                    min_value = distance_for_layouts[layout]
                    most_efficient_layout = layout

            print(
                f"The most optimal layout is {most_efficient_layout}, and corresponding distance is {min_value}"
            )

    else:
        plot1(x, y, grid_size)  # plotting i -a flag not given
