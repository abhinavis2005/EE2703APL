...
def genFreq(inpStr: str, layout, characters: dict) -> tuple[np.array, np.array]:
    ...
    for char in inpStr:
        ...
        for key in keys:  # for each key to be pressed
            ...
            x.append(
                layout[key]["pos"][0] + 0.5
            ) 
            y.append(
                layout[key]["pos"][1] + 0.5
            ) 
...
def plot(
    ...
    """
    Optimized function to plot a heatmap animation.
    """
    heatmap = np.zeros(grid_size)  # heatmap matrix for each pixel
    x_min, x_max = 0, 14.5  # coordinate limits
    y_min, y_max = 1, 5
    ...
    def update(frame):
        ...
        # Add the frequency to the heatmap within the circular region
        influence = (distance <= radius) * gradual_decay(distance, radius)
        heatmap += influence

        # Update the artist's heatmap data and redraw
        im.set_array(heatmap.T)
     # Redraw the artist for each frame

        return [im]

    # Create the animation
    ani = FuncAnimation(
        fig, update, frames=len(x), interval=100, blit=False, repeat=False
    )
    
    # Save the animation as a GIF
    #ani.save('optimized_animation.gif', writer='pillow', fps=30)

    plt.show()  # Show the plot after saving the animation
...
if __name__ == "__main__":
    ...
    genKeyboardLayout(ax, qwerty_layout.keys)
